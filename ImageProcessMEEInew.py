...
def process_eye(EyeImage):
   ...
   #this variable will be used to detect when the mouse button is click and lifted. 
   #only there we can be sure that the user added a new point (i.e, didn't just 
   #hold the mouse clicked in the same position)                                                            
    click_lift_detector=np.array([0])
    time_counter=1
    while(True):
        cv2.imshow("EyeImage", temp_image)
        cv2.waitKey(100)
        click_lift_detector=np.append(click_lift_detector,[GetCoordsonClickEye.click])
        ...
        #verify that the user actually pressed and lifted the mouse button, if this 
        #happened then this implies that a new point was added
        if (click_lift_detector[time_counter] -   click_lift_detector[time_counter-1]) < 0 : 
            clickCounter += 1
    ...
    while(True):
        ...
        if ShouldModify == 1:  
            ...
            if IsPointLifted == 1:
                ...
                NewKeyPressed = cv2.waitKey(50) 
                if  NewKeyPressed & 0xFF == ord('z'):
                    temp_image=image.copy()
                
                if GetCoordsonClick.click == 2 :
...
def main(path,k):
    ...
    if os.path.isfile(__location__ + '\\include\\shape_predictor_68_face_landmarks.dat'):
        #if it exists then load it 
        predictor = shape_predictor(__location__ + '\\include\\shape_predictor_68_face_landmarks.dat')
    else:
